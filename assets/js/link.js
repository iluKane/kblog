    ...
    function createKHeader(homeData) {
        ...
        kheader.innerHTML = `
            <div class="header-nav">
                <div class="header-nav-item" onclick="window.location.href='${homeData.url}'">
                    ${homeData.title}
                </div>
    ...
    function addDropdownEventListeners(homeData) {
        ...
        function navItem(index1, index2) {
            ...
            homeData.items.forEach((item, index) => {
                turnResults1.innerHTML +=
                    `<div class="turn-item" data-index="${index}">${item.title}</div>`;
            });

            homeData.items[index1].items.forEach((item, subIndex) => {
                turnResults2.innerHTML +=
                    `<div class="turn-item1" data-index="${subIndex}">${item.title}</div>`;
            });
        ...
        turnResults1.addEventListener("click", function (event) {
            if (event.target.classList.contains("turn-item")) {
                ...
                turnIcon1.style.transform = turnIcon1.style.transform === "rotateZ(90deg)" ? "rotateZ(0deg)" : "rotateZ(90deg)";
        ...
        turnIcon1.addEventListener("click", () => {
            turnResults1.style.display = turnResults1.style.display === "block" ? "none" : "block";
        ...
        turnIcon2.addEventListener("click", () => {
            turnResults2.style.display = turnResults2.style.display === "block" ? "none" : "block";
            turnResults1.style.display = "none";
            turnIcon2.style.transform = turnIcon2.style.transform === "rotateZ(90deg)" ? "rotateZ(0deg)" : "rotateZ(90deg)";
